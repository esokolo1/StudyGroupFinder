...
let init = (app) => {
  ...
  app.createCalendar = function () {
    axios.get(calendar_url).then(function (r) {
      app.vue.month = r.data.month;
      app.vue.month_name = r.data.month_name;
      app.vue.year = r.data.year;
      app.vue.weeks1 = r.data.weeks;
  ...
  app.getEvents = function (month, cell, year) {
    ...
    axios
      ...
      .then(function (response) {
        ...
        // sort by starttime
        app.vue.upcoming_sessions.sort((a, b) =>
          a.session_time > b.session_time ? 1 : -1
        );
      });
  };
      ...
      .then(function (result) {
        ...
        for (let i = 0; i < app.vue.results.length; i++) {
          ...
          // if num_students < max_num_students, display "enroll" button on search page
            ...
            app.vue.results[i].num_students <
            app.vue.results[i].max_num_students
          ) {
  ...
  app.get_comments = function (id) {
    axios.get(get_comments_url, { params: { id: id } }).then(function (result) {
  ...
  app.add_comment = function (id) {
    axios
      .post(add_comment_url, { id: id, new_comment: app.vue.new_comment })
      .then(function (result) {
        app.vue.new_comment = "";
        app.get_comments(id);
      });
